...
export default async function handler(req, res) {
  if (req.method === "GET") { res.status(200).json({ ok: true, version: "v23-sid-fallback" }); return; }
  ...
  try {
    ...
    try {
      ...
      if (!sid) { // fabrike yon id estab si App.jsx pa voye youn
        const fu = (messages.find((mm) => mm.role === "user" && !String(mm.content || "").startsWith("(Sistèm:")) || {}).content || "";
        let h = 0; const base = (ctx.program || "") + "|" + String(fu).slice(0, 80);
        for (let i = 0; i < base.length; i++) { h = ((h << 5) - h + base.charCodeAt(i)) | 0; }
        sid = "s" + Math.abs(h).toString(36);
      }
      await saveCarlaChat(ctx.supabaseUrl, ctx.supabaseKey, sid, ctx.program, nm, fullT);
    } catch (e) {}
